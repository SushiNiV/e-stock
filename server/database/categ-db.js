...
app.post('/categories', authenticateToken, validateStoreOwnership, (req, res) => {
  ...
  // Ensure the category code's letter part is 4 characters long, append 'X' if necessary
  ...
  db.query(checkDuplicateQuery, [trimmedName, storeId], (err, result) => {
    ...
    // Get all category codes for the store
    ...
    db.query(availableNumberQuery, [storeId], (err, existingCategories) => {
      ...
      const usedNumbers = existingCategories.map(category => {
        const number = parseInt(category.CategoryCode.substring(5), 10); // Extract the numeric part of the code
        return number;
      });

      // Find the first gap in the sequence or the next available number
      let nextNumber = 1;

      // Loop through the numbers and find the first missing number
      while (usedNumbers.includes(nextNumber)) {
        nextNumber++;
      }

      // Format the category code as CXXXX###, ensuring the number is padded to 3 digits
