...
function EditProduct({ onClose, product, onUpdateProduct }) {
  ...
  useEffect(() => {
    ...
    if (token && product) fetchCategories();
  }, [token, product]);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!storeId || !product?.ProductID) {
    ...
    try {
      setIsLoading(true);
      const res = await fetch(`http://localhost:3004/products/${product.ProductID}`, {
        method: 'PUT',
        headers: {
    ...
    <div className="a-wrapper">
      <div className="a-content-wrapper">
        ...
        <div className="a-container grid">
          <div className="a-box">
            <h1 className="a-title">Edit Product</h1>
